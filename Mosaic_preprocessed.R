...
create_photomosaic <- function(target_image_path, small_images, output_file, 
  ...
  # do this if you want a square shaped output image
  if(force_square){
  ...
  tile_info <- data.frame(x = integer(), y = integer(), image_path = character(), 
                          stringsAsFactors = FALSE)
  
  # Check if intermediate results file exists and load it if it does. This will 
  # let the program skip tiles which have already been processed
  if (file.exists(intermediate_results_file)) {
    
    tile_info <- read_csv(intermediate_results_file)
    
    # Save already processed tiles in a list, these will be skipped later on
    processed_tiles <- paste(tile_info$x, tile_info$y, sep = "_")
    
  } else {
    
    # the program has not run before, create a new list
  ...
  for (y in seq(1, target_height, by = tile_size)) {
    for (x in seq(1, target_width, by = tile_size)) {
      ...
      # if tile has been processed before, skip this tile
      ...
      mosaic <- image_composite(mosaic, best_match, 
                                offset = paste0("+", x-1, "+", y-1))
      
      # Save the tile information
      tile_info <- tile_info %>% add_row(x = x-1, y = y-1, 
  ...
  # TODO tile_info needs to be reused to speed up the process
